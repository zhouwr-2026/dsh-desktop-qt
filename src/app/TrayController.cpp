...
TrayController::TrayController(QObject* parent) : QObject(parent) {
    ...
    menu_->addSeparator();
    // "重启"：重新拉起当前可执行文件并退出，后端动作跟随确认对话框
    // （未运行时始终拉起；运行时按勾选决定是否重启后端）。
    restartAct_ = menu_->addAction(tr("重启"));
    menu_->addSeparator();
...
void TrayController::bind(DshWindow* window,
    ...
    connect(restartAct_, &QAction::triggered, this,
...
void TrayController::setMenuIcons() {
    ...
    if (restartAct_) restartAct_->setIcon(fromTheme(QStringLiteral("system-reboot")));
